    ...
    Parent {"Esp8266::Processor"}
{
    ...
    /*
     * ESP8266 is the C2I master when it directly talks to a sensor.
     * In part 3 we mostly use USART toward the MUX, but we keep this behavior
     * because the board also supports the part 1 direct C2I connection.
     */
    ...
    public:
        ...
        init(Byte address) override
        {
            m_address    = address;
            m_addressing = true;
            m_board->gpio().sda.write(address);
        }
        ...
        run(Gpio& gpio) override
        {
            ...
            if(!m_started)
            {
                ...
                if(gpio.sda.readBit() != Bit::One) return;
